...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        if not self.check_duplicate_nodes(id, name):
            self.nodes.append((id,name))

    def check_duplicate_nodes(self, id: str, name: str) -> bool:
        """
        determine if tuple (id, name) already exists in list self.nodes
        """
        result = False
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        if not self.check_duplicate_edges(source,target):
            self.edges.append((source,target))
    
    def check_duplicate_edges(self, source: str, target: str) -> bool:
        """
        determine if tuple (id, name) already exists in list self.nodes
        """
        result = False
        for edge in self.edges:
            if edge == (source,target) or edge == (target,source):
                return True
        return result
    ...
    def max_degree_nodes(self) -> dict:
        ...
        or {'a': 22, 'b': 22}
        """
        node_ids = [self.nodes[i][0] for i in range(len(self.nodes))]
        nodes_dict =  {node_id:0 for node_id in node_ids}


        for node_id,degree in nodes_dict.items(): 
            #Iterate through edges and count how many times the id is found
            for source,target in self.edges.copy():
                if node_id==source or node_id==target:
                    nodes_dict[node_id]+=1
        max_value_keys = {key:value for key,value in nodes_dict.items() if nodes_dict[key] == max(nodes_dict.values())}
        max_value_keys[list(max_value_keys.keys())[0]]+=1
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        """
        result = []
        request = requests.get(f"https://api.themoviedb.org/3/movie/{movie_id}/credits?api_key={self.api_key}&language=en-US")
        movie_cast = json.loads(request.content)['cast']
        ...
        for cast_member in movie_cast:
            ...
            if limit is not None:
                if cast_member['order'] >= 0 and cast_member['order'] < limit:
                    result.append(cast_member)
            
            if exclude_ids is None and limit is None:
                result.append(cast_member)
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        """
        result = []
        request = requests.get(f"https://api.themoviedb.org/3/person/{person_id}/movie_credits?api_key={self.api_key}&language=en-US")
        movie_credits = json.loads(request.content)['cast']
        if vote_avg_threshold is not None:
            for movie in movie_credits:
                ...
                if movie['vote_average'] >= vote_avg_threshold:
                    sub_dict['id']=movie['id']
                    sub_dict['title']=movie['title']
                    sub_dict['vote_avg']=movie['vote_average']
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#734c24e5-b8ca-4863-9c62-72255416291c"
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='b5ace75dd6102d0afcc50ef96474ac05')

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK
    movie_credits = tmdb_api_utils.get_movie_credits_for_person('2975',8)
    for movie in movie_credits:
        movie_cast = tmdb_api_utils.get_movie_cast(movie['id'],3)
        for cast_member in movie_cast:
    ...
    #2 iterations
    exclude_ids = ['2975']
    for iter_num in range(0,2):
        current_nodes = graph.nodes.copy() 
        for node in current_nodes:
            if node[0] not in exclude_ids:
                movie_credits = tmdb_api_utils.get_movie_credits_for_person(node[0],8)
                exclude_ids.append(str(node[0]))
                for movie in movie_credits:
                    movie_cast = tmdb_api_utils.get_movie_cast(movie['id'],3,exclude_ids)
                    for cast_member in movie_cast:
                        graph.add_node(str(cast_member['id']),str(cast_member['name']))
                        graph.add_edge(str(node[0]),str(cast_member['id']))
                        


    tmdb_api_utils.get_movie_cast(329)
